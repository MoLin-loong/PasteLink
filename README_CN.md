...
连接成功后，左上角“已连接”右侧会显示当前电脑名称，默认采用 Windows 用户名。点击名称可以自定义别名；别名保存在手机浏览器本地，并按电脑分别记录，方便在多台设备之间区分。
